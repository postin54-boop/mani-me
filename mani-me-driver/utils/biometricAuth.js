...
const BIOMETRIC_ENABLED_KEY = '@driver_biometric_enabled';

// Simple logger fallback if utils/logger doesn't exist
const logger = {
  log: (...args) => console.log('[Biometric]', ...args),
  warn: (...args) => console.warn('[Biometric]', ...args),
  error: (...args) => console.error('[Biometric]', ...args),
};
...
export const attemptBiometricAuth = async () => {
  ...
  const result = await authenticateWithBiometrics(`Unlock Mani Me Driver with ${biometricName}`);
